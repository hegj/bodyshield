...
//		initPlayWork();
     ...
     * @param sounds
     */
    private void initPlayWork(int sources) {
        if (sp != null) {
            sp.release();
        }
        if (audioMgr == null) {
        ...
        load = sp.load(this, sources, 1);
    }

    public void playSound() {
        initPlayWork(R.raw.sounds);
    ...
    private boolean kickQuiltWarningFlag;
    private boolean isKickQuiltWarning;
    private int kickQuiltWarningTimesIndex = 0;
    private int kickQuiltWarningTimes = 3;
    private Runnable kickQuiltWarningRunnable = new Runnable() {
        @Override
        public void run() {
            if (kickQuiltWarningTimesIndex < kickQuiltWarningTimes) {
                playKickQuiltWarning();
                alertByVibrate();
                handler.postDelayed(this, 60 * 1000);
                kickQuiltWarningTimesIndex++;
            } else {
                kickQuiltWarningTimesIndex = 0;
                isKickQuiltWarning = false;
                kickQuiltWarningFlag = false;
            }
        }
    };

    private void playKickQuiltWarning() {
        initPlayWork(R.raw.kick_quilt_warning);
        sp.setOnLoadCompleteListener(new SoundPool.OnLoadCompleteListener() {
            @Override
            public void onLoadComplete(SoundPool soundPool, int sampleId, int status) {
                float volumnCurrent = audioMgr.getStreamVolume(AudioManager.STREAM_MUSIC);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (PresentDataResponse.ACTION_REALTIME_TEMPRETURE.equals(action)) {
                ...
                if (device != null && device.getAddress().equals(address)) {
                    ...
                    // 需要先判断是否大于36 ,然后再判断是否小于了34度
                    if (y > 36) {
                        // 如果大于36
                        kickQuiltWarningFlag = true;
                    }
                    if (kickQuiltWarningFlag && y < 34 && !isKickQuiltWarning) {
                        // 报警
                        handler.postDelayed(kickQuiltWarningRunnable, 1000);
                        isKickQuiltWarning = true;
                    }
                /*	if (y>34){
						isKickQuiltWarning=false;
						kickQuiltWarningFlag=false;
